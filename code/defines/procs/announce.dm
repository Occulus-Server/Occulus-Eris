	...
	if (join_message && SSticker.current_state == GAME_STATE_PLAYING && SSjob.ShouldCreateRecords(rank))
		if(issilicon(character))
			...
			if (character.client)
				...
				global_announcer.autosay("[character.real_name], [rank], [join_message].", ANNOUNCER_NAME, use_text_to_speech = TRUE)	// This should not trigger -- but it's here as an emergency fallback
			// OCCULUS EDIT END
