		...
		if(!BM.client)
			for(var/mob/observer/ghost/G in GLOB.player_list)
				if(G.can_reenter_corpse && G.mind == BM.mind)
					if(alert(G, "Somebody is attempting to put your brain in an MMI. Would you like to return to it?","Become brain","OH YES","No") == "OH YES") // Occulus Edit - Ask ghost if they want to be MMI'd
						G.reenter_corpse()
						break
			if(!BM.client)
