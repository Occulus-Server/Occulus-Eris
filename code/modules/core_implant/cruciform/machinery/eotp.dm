...
/obj/machinery/power/eotp
	...
	var/list/rewards = list(ODDITY, STAT_BUFF, MATERIAL_REWARD, ENERGY_REWARD) // OCCULUS EDIT - Disable the antag radar and positive breakdown rewards
...
/obj/machinery/power/eotp/proc/power_release()
	...
	if(type_release == ALERT)
		...
		visible_message(SPAN_NOTICE("The [_item.name] appears in a flash of light near the [src]!")) // Occulus Edit - Lore reflavoring
		rewards -= ODDITY
