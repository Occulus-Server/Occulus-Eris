...
/datum/reagent/drink/coffee/affect_ingest(mob/living/carbon/M, alien, effect_multiplier)
	...
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
		var/obj/item/organ/internal/kidneys/K = H.random_organ_by_process(OP_KIDNEYS) // Occulus Edit - One kidney
		if(istype(K))
			if(K.is_bruised())
...
/datum/reagent/alcohol/ntcahors/activated // Occulus Addition Start
...
/datum/reagent/alcohol/ntcahors/activated/affect_ingest(mob/living/carbon/M, alien, effect_multiplier)
	...
	M.adjustBruteLoss(- 1 * effect_multiplier) // Occulus Addition End
