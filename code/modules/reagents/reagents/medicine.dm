...
//	M.add_chemical_effect(CE_ANTITOX, 1) Occulus Edit
...
/datum/reagent/medicine/clonexadone/affect_blood(mob/living/carbon/M, alien, effect_multiplier)
	if(M.bodytemperature < 170)
		...
		M.adjustToxLoss(-(3 + (M.getToxLoss() * 0.05)) * effect_multiplier) // Occulus Edit Start - Gives Medbay a way to unhusk people
		M.add_chemical_effect(CE_PULSE, -2)
		if(M.stat == DEAD)
			M.timeofdeath += 20
		// if(ishuman(M))
