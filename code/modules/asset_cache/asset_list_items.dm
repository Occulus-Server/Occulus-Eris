	...
	for(var/name in SScraft.categories)
		for(var/datum/craft_recipe/CR in SScraft.categories[name])
			if(CR.result)
				...
				#ifdef UNIT_TESTS
				if(!(icon_state in icon_states(icon_file)))
					warning("crafting result [CR] with icon '[icon_file]' missing state '[icon_state]'")
					continue
				#endif
				var/icon/I = icon(icon_file, icon_state, SOUTH)
...
/datum/asset/simple/tool_upgrades/register()
	for(var/type in subtypesof(/obj/item/tool_upgrade))
		...
		#ifdef UNIT_TESTS
		if(!(icon_state in icon_states(icon_file)))
			warning("tool upgrade [type] with icon '[icon_file]' missing state '[icon_state]'")
			continue
		#endif
...
/datum/asset/simple/perks/register()
	for(var/type in subtypesof(/datum/perk))
		...
		#ifdef UNIT_TESTS
		if(!(icon_state in icon_states(icon_file)))
			warning("perks [type] with icon '[icon_file]' missing state '[icon_state]'")
			continue
		#endif
		var/icon/I = icon(icon_file, icon_state, SOUTH)
