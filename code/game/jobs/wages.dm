...
/datum/job/proc/one_time_payment(var/custom_factor = 1)
	if (initial_balance != -1)
		return round (initial_balance * (rand(14, 17) / 10) * custom_factor) // OCCULUS EDIT - Nobles get 5x the starting balance
	return round(wage * (rand(30, 70) / 10) * custom_factor)	// OCCULUS EDIT ^
