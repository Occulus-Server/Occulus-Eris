...
/proc/do_payday()
	...
	for(var/datum/money_account/A in personal_accounts)
		...
		if(amount_to_pay <= EA.money)
			transfer_funds(EA, A, "Payroll Funding", "CEV Northern Light payroll system", amount_to_pay)
			paid_internal += amount_to_pay
			ED.total_debt -= A.debt
			A.debt = 0
			if(R)
	...
	command_announcement.Announce("Half hourly crew wages have been paid, please check your email for details. In total the crew of CEV Northern Light have earned [total_paid] credits, including [paid_external] credits from external sources.\n Please contact your Department Heads in case of errors or missing payments.", "Dispensation")
