...
/datum/perk/oddity/toxic_revenger/on_process()
	...
	if(world.time < initial_time + cooldown)
		return
	initial_time = world.time
	for(var/mob/living/carbon/human/H in viewers(2, holder)) // Occulus Edit - Nerving the radius
		...
		// H.reagents?.add_reagent("toxin", 5) Occulus Removal - Removing the damage
		H.emote("cough")
