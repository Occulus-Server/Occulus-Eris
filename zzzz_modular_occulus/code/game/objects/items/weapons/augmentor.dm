...
/obj/item/augmentor/New()
	..()
	if(ispath(augmentpath))
		augment = new augmentpath
		update_icon()

/obj/item/augmentor/attack_self(mob/user)
	if(!augment)
		return ..()
...
/obj/item/augmentor/attack(mob/living/M, mob/living/user)
	if(!istype(M) || !augment)
		return
...
/obj/item/augmentor/on_update_icon()
	if(augment)
		icon_state = "augmentor"
	else
...
/obj/item/organ_module/attackby(obj/item/I, mob/user)
	..()
	if (istype(I, /obj/item/augmentor))
		var/obj/item/augmentor/M = I
		if(!M.augment && user.unEquip(src, M))
			M.augment = src
			M.update_icon()
			matter = list(MATERIAL_STEEL = 1, MATERIAL_PLASTIC = 1, MATERIAL_PLATINUM = 1)

obj/item/augmentor/debug
	name = "Debugmentor"
	desc = "This is a debugging tool"
	augmentpath = /obj/item/organ_module/debugger

/obj/item/augmentor/e_armblade
	...
	matter = list(MATERIAL_PLASTEEL = 10, MATERIAL_STEEL = 1, MATERIAL_PLASTIC = 1, MATERIAL_PLATINUM = 1, MATERIAL_PLASMAGLASS = 4, MATERIAL_TRITIUM = 1)

/obj/item/augmentor/hurricane
