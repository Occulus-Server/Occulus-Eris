...
/mob/living/simple_animal/hostile/siren/turret/ForgetTarget(atom/T)
	...
	if(T == src.target_mob)	//if mob is currently the immediate target, lose target
		if(targetting)
			src.target_mob.overlays -= image('zzzz_modular_occulus/icons/mob/siren/general.dmi', "targetlocked_indicator")
		else
			src.target_mob.overlays -= image('zzzz_modular_occulus/icons/mob/siren/general.dmi', "targetting_indicator")
		LoseTarget()
...
/mob/living/simple_animal/hostile/siren/turret/proc/BeamStartup(mob/living/living_target)//It's more like requiem than final spark
	if(combatant_state == TURRET_STATE_WARMUP && target_mob)
		...
		living_target.overlays -= image('zzzz_modular_occulus/icons/mob/siren/general.dmi', "targetting_indicator")
		...
		if(get_dist(src,living_target) > 7)
			...
			living_target.overlays -= image('zzzz_modular_occulus/icons/mob/siren/general.dmi', "targetlocked_indicator")
			living_target.update_icons()
			AttackRecovery()
		else
