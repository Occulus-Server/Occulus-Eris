...
/obj/item/trash/plate/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W,/obj/item/weapon/material/shard) || istype(W,/obj/item/weapon/reagent_containers/food/snacks))
		var/obj/item/weapon/reagent_containers/food/snacks/meal/M = new(get_turf(src))
		M.attackby(W,user)
		qdel(src)
	..()
...
/obj/item/weapon/reagent_containers/food/snacks/meal
	name = "meal"
...
/obj/item/weapon/reagent_containers/food/snacks/meal/attackby(obj/item/W as obj, mob/user as mob)

	var/sandwich_limit = 3 //Lower limit than sandwiches because it's intended to contain more filling food.

	if(src.contents.len > sandwich_limit)
		to_chat(user, "\red The [src] plate is full!")
		return
	else if(istype(W, /obj/item/weapon/reagent_containers/food/snacks/meal) || istype(W, /obj/item/weapon/reagent_containers/food/snacks/csalad)) //Quick check to not make these recursive.
		to_chat(user, "\red The [src] is too small to hold the [W]!")
		return
...
/obj/item/weapon/reagent_containers/food/snacks/meal/proc/update()
...
/obj/item/weapon/reagent_containers/food/snacks/meal/Destroy()
	for(var/obj/item/O in ingredients)
		qdel(O)
	. = ..()

/obj/item/weapon/reagent_containers/food/snacks/meal/examine(mob/user)
	..(user)
	var/obj/item/O = pick(contents)
	to_chat(user, SPAN_NOTICE("You think you can see [O.name]. Yum."))

/obj/item/weapon/reagent_containers/food/snacks/meal/attack(mob/M as mob, mob/user as mob, def_zone)
	var/obj/item/shard
