	...
	if(href_list["choice"])
		switch(href_list["choice"])
			...
			if("link_account")
				...
				if(linked_account)
					if(linked_account.suspended)
						linked_account = null
						src.visible_message("<span class='warning'>Account has been suspended.</span>")
				else
			...
			if("custom_order")
				...
				src.visible_message("[transaction_purpose]: [t_amount] Thaler\s.")
			if("set_amount")
			...
			if("reset_log")
				...
				to_chat(usr, "<span class='notice'>Transaction log reset.</span>")
	updateDialog()
...
/obj/item/device/retail_scanner/proc/confirm(var/obj/item/I)
	if(confirm_item == I)
		...
		confirm_item = I
		src.visible_message("<b>Total price:</b> [transaction_amount] Thaler\s. Swipe again to confirm.")
		playsound(src, 'sound/machines/twobeep.ogg', 25)
		return 0
...
/obj/item/device/retail_scanner/proc/scan_card(var/obj/item/weapon/card/id/I, var/obj/item/ID_container)
	...
	if (!linked_account)
		usr.visible_message("<span class='warning'>Unable to connect to linked account.</span>")
		return
	...
	if(check_account())
		...
		if(!D)
			src.visible_message("<span class='warning'>Unable to access account. Check security settings and try again.</span>")
		else
			if(D.suspended)
				src.visible_message("<span class='warning'>Your account has been suspended.</span>")
			else
				if(transaction_amount > D.money)
					src.visible_message("<span class='warning'>Not enough funds.</span>")
				else
					// Transfer the money
...
/obj/item/device/retail_scanner/proc/scan_item_price(var/obj/O)
	if(!istype(O))	return
	if(item_list.len > 10)
		src.visible_message("<span class='warning'>Only up to ten different items allowed per purchase.</span>")
		return

	// First check if item has a valid price
	var/price = O.get_item_cost()
	if(isnull(price))
		src.visible_message("<span class='warning'>Unable to find item in database.</span>")
		return
	// Call out item cost
	src.visible_message("\A [O]: [price ? "[price] Thaler\s" : "free of charge"].")
...
/obj/item/device/retail_scanner/proc/check_account()
	...
	if(linked_account.suspended)
		src.visible_message("<span class='warning'>Connected account has been suspended.</span>")
		return 0
	return 1
...
/obj/item/device/retail_scanner/proc/transaction_complete()
	...
	src.visible_message("<span class='notice'>Transaction complete.</span>")
