...
// RAZEK STOP TOUCHING THE VARS WRITE THE PROCS (delete this later)
...
/obj/item/weapon/gun/projectile/automatic/white_nail/Process() // generates ammo in the loaded mag
	current_ammo = src.get_ammo()
	
	rebuild_tick++
	if(rebuild_tick < rebuild_time) return FALSE // has tick reached 100%?
	rebuild_tick = 0
	...
	if(ammo_magazine.stored_ammo.len >=  ammo_magazine.max_ammo) // plays a sound when the mag is full
		src.visible_message("\ The White Nail chimes, indicating the magazine is full.", 1)
		playsound(src.loc, refill_sound, 75, 1)
